...
class ChatService {
  async getChatSessions(projectId: string): Promise<BasicSession[]> {
    const response = await apiClient.get<{ sessions: BasicSession[] }>(`/chat/projects/${projectId}/sessions`);
    return response.sessions || [];
  }

  async getChatSession(sessionId: string, projectId: string): Promise<ChatSession> {
    return apiClient.post<ChatSession>(`/chat/projects/${projectId}/sessions/${sessionId}`, {});
  }

  async createChatSession(data: CreateSessionData): Promise<ChatSession> {
    return apiClient.post<ChatSession>(`/chat/projects/${data.project_id}/sessions`, data);
  }
  ...
  async deleteSession(sessionId: string, projectId: string): Promise<void> {
    return apiClient.delete(`/chat/projects/${projectId}/sessions/${sessionId}`);
  }

  async updateSessionTitle(sessionId: string, projectId: string, title: string): Promise<ChatSession> {
    return apiClient.put<ChatSession>(`/chat/projects/${projectId}/sessions/${sessionId}`, { title });
  }
