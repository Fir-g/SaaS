...
const Workspace = () => {
  ...
  useEffect(() => {
    ...
    if (path.includes('/workspace/files/')) setActiveView('files');
    else if (path.includes('/workspace/extracted-data/')) setActiveView('data');
    else if (path.includes('/workspace/chat/')) setActiveView('chat');
  }, [location.pathname]);
  ...
  const handleQuickAction = (action: string) => {
    ...
    switch (action) {
      case "add_file":
        setActiveView("files");
        if (projectId) navigate(`/workspace/files/${projectId}`, { replace: true, state: location.state });
        break;
      case "invite_member":
      ...
      case "new_chat":
        setActiveView("chat");
        if (projectId) navigate(`/workspace/chat/${projectId}`, { replace: true, state: location.state });
        break;
    }
  };
  ...
  const handleViewChange = (view: ActiveView) => {
    setActiveView(view);
    if (!projectId) return;
    const base = view === 'files' ? '/workspace/files'
      : view === 'data' ? '/workspace/extracted-data'
      : '/workspace/chat';
    navigate(`${base}/${projectId}`, { replace: true, state: location.state });
  };
  ...
  return (
    <SidebarProvider>
      <div className="min-h-screen flex w-full bg-background">
        ...
        <main 
          className={cn(
            "flex-1 h-screen pt-12 overflow-hidden transition-all duration-300 ease-in-out",
            sidebarCollapsed ? "pl-16" : "pl-80"
          )}
        >
          <div className="h-full w-full overflow-hidden">{renderActiveView()}</div>
        </main>
      </div>
    </SidebarProvider>
  );
