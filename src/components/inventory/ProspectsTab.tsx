...
import { RefreshCw, AlertCircle, X, ChevronLeft, ChevronRight, CalendarDays, PhoneCall, Check } from 'lucide-react';
...
const ProspectsTab: React.FC<ProspectsTabProps> = ({ 
  vehicleTypeOptions, 
  ...
  // User and PSA preferences state
  const { user } = useUser();
  const [psaPreferencesLoaded, setPsaPreferencesLoaded] = useState(false);
    ...
    try {
      ...
      if (response.success) {
        ...
        toast({
          ...
          description: "Unable to load prospects data. Please try again.",
          variant: "destructive",
        });
      }
    } catch (error) {
  ...
  }, [currentPage, fleetOwnerFilter, vehicleTypeFilter, performanceFilter, selectedOriginClusters, selectedDestinationClusters, toast, token]);

  const fetchPsaPreferences = useCallback(async () => {
    if (!user?.primaryEmailAddress?.emailAddress || psaPreferencesLoaded) {
      return;
    }

    try {
      const userEmail = user.primaryEmailAddress.emailAddress;
      const response = await apiService.getPsaClusterMappingByEmail(userEmail, token);
      
      if (response.success && response.value) {
        const psaData = response.value;
        
        // Extract origin and destination clusters from the response
        if (psaData.originClusters && Array.isArray(psaData.originClusters)) {
          
          setSelectedOriginClusters(psaData.originClusters);
        }
        
        if (psaData.destinationClusters && Array.isArray(psaData.destinationClusters)) {
          
          setSelectedDestinationClusters(psaData.destinationClusters);
        }
        
        setPsaPreferencesLoaded(true);
  ...
  }, [user, token, psaPreferencesLoaded, toast]);
  ...
  useEffect(() => {}, [fleetOwnerFilter, vehicleTypeFilter, performanceFilter, selectedOriginClusters, selectedDestinationClusters]);

  // Specific useEffect for vehicle type filter changes
  useEffect(() => {
    if (vehicleTypeFilter !== 'all') {
      fetchProspectsData();
    }
  }, [vehicleTypeFilter, fetchProspectsData]);

  // Fetch PSA preferences on component mount
  useEffect(() => {
    fetchPsaPreferences();
  }, [fetchPsaPreferences]);
    ...
    <div className="space-y-4 md:space-y-6">
      ...
      <div className="rounded-md border bg-card p-3">
        <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-2">
                ...
                <div className="p-2 border-b">
                  <Input
                    ...
                    onChange={(e) => {
                      ...
                      if (value.length >= 3) { handleFleetOwnerSearch(value); } else { setFleetOwnerOptions([]); }
                    }}
            ...
            <Select value={vehicleTypeFilter} onValueChange={(value) => {
              ...
              <SelectContent>
                <SelectItem value="all">All Types</SelectItem>
                                    {vehicleTypeOptions.map(type => (<SelectItem key={type.value} value={type.text}>{type.text}</SelectItem>))}
              </SelectContent>
            </Select>
          </div>
          <div className="space-y-1">
            ...
            <div className="relative">
              <Select value={'all'} onValueChange={(value) => {
                if (value === 'all') { setSelectedOriginClusters([]); return; }
                ...
                <SelectContent>
                  ...
                  {originClusterOptions.filter(o => originClusterSearchTerm === '' || o.text.toLowerCase().includes(originClusterSearchTerm.toLowerCase())).map(o => (
              ...
              <div className="flex flex-wrap gap-1 pt-1">
                {selectedOriginClusters.map((cluster) => (
                  <Badge key={cluster} variant="secondary" className="text-xs px-2 py-1 h-6 flex items-center gap-1">
                    ...
                    <button onClick={() => setSelectedOriginClusters(selectedOriginClusters.filter(c => c !== cluster))} className="hover:text-destructive text-xs font-medium" title="Remove">×</button>
                  </Badge>
                ))}
              </div>
            )}
          ...
          <div className="space-y-1">
            ...
            <div className="relative">
              <Select value={'all'} onValueChange={(value) => {
                if (value === 'all') { setSelectedDestinationClusters([]); return; }
                ...
                <SelectContent>
                  ...
                  {destinationClusterOptions.filter(o => destinationClusterSearchTerm === '' || o.text.toLowerCase().includes(destinationClusterSearchTerm.toLowerCase())).map(o => (
                    <SelectItem key={o.value} value={o.value}>{o.text}</SelectItem>
                  ))}
                </SelectContent>
              </Select>
              ...
              <div className="flex flex-wrap gap-1 pt-1">
                {selectedDestinationClusters.map((cluster) => (
                  <Badge key={cluster} variant="secondary" className="text-xs px-2 py-1 h-6 flex items-center gap-1">
                    ...
                    <button onClick={() => setSelectedDestinationClusters(selectedDestinationClusters.filter(c => c !== cluster))} className="hover:text-destructive text-xs font-medium" title="Remove">×</button>
                  </Badge>
                ))}
              </div>
            )}
        ...
        <Table className="table-fixed">
          <TableHeader className="sticky top-0 z-20 bg-background border-b">
            <TableRow className="border-b">
              ...
              <TableHead className="sticky top-0 z-20 bg-background text-left font-bold text-base">Score Card</TableHead>
              <TableHead className="sticky top-0 z-20 bg-background text-left font-bold text-base">Actions</TableHead>
            </TableRow>
      ...
      <div className="flex items-center justify-between">
        <div className="text-sm text-muted-foreground">
          ...
          {(fleetOwnerFilter !== 'all' || vehicleTypeFilter !== 'all' || performanceFilter !== 'all' || selectedOriginClusters.length > 0 || selectedDestinationClusters.length > 0) && (
