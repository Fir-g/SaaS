...
interface FileCardProps {
  ...
  onShowSummary: (file: FileData) => void;
}
      ...
      <CardContent className="p-2 px-4">
        <div className="flex items-start justify-between mb-1">
          ...
          <div className="flex items-center space-x-2 mt-2">
            ...
            <Badge
              className={`${getStatusColor(file.status)} mt-2 px-3 py-1 text-xs font-semibold rounded-full`}
            >
                ...
                <Tooltip>
                  <TooltipTrigger asChild>
                    <Button
                      variant="secondary"
                      size="sm"
                      className="h-6 mt-2 rounded-full bg-indigo-50 text-indigo-700 border border-indigo-200 hover:bg-indigo-100 hover:text-indigo-800"
                      onClick={() => onShowSummary(file)}
                    >
                      SUMMARY
                    </Button>
                  </TooltipTrigger>
                  <TooltipContent>
                    View summary of this file
                  </TooltipContent>
                </Tooltip>
              </TooltipProvider>
            )}
            <Button
              variant="ghost"
              size="sm"
              className="h-8 w-8 p-0 text-red-400 hover:text-red-600 hover:bg-red-50 mt-2"
