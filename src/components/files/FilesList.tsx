...
export const FilesList = ({ files, loading, onRequestDelete }: FilesListProps) => {
  ...
  const openSummary = (file: FileData) => {
    if (!file.file_summary) return;
    
    setSummaryContent(file.file_summary);
    setSummaryOpen(true);
  };
    ...
    <div className="flex-1 overflow-auto p-4">
      ...
      <Dialog open={summaryOpen} onOpenChange={setSummaryOpen}>
        <DialogContent className="max-w-5xl max-h-[80vh] overflow-auto">
          <MarkdownComponent content={summaryContent} />
        </DialogContent>
      </Dialog>
    </div>
  );
