...
const SplitBarChart: React.FC<SplitBarChartProps> = ({
  ...
  const titleId = `chart-${title.replace(/\s+/g, "-").toLowerCase()}`;
  ...
  return (
    <div
      ...
      className="relative bg-white border border-gray-200 rounded-2xl p-4 shadow-sm hover:shadow-lg transition-all duration-300 hover:border-gray-300"
    >
      {/* Header */}
      <div className="flex items-center justify-between mb-3">
        <div className="flex items-center gap-3">
          <div
            className={`p-2 ${config.bgColor} rounded-xl ${config.borderColor} border`}
          >
            {config.icon}
          </div>
          <div>
            ...
            <p className="text-xs text-gray-500">
              Top {Math.min(10, data.length)} entries
            </p>
          </div>
        </div>
        <div
          className={`px-3 py-1.5 ${config.bgColor} rounded-full border ${config.borderColor}`}
        >
      ...
      <div className="min-h-[200px]">
        {isLoading ? (
          <div className="space-y-2">
            ...
            {[...Array(4)].map((_, i) => (
              <div key={i} className="flex items-center gap-2 h-8" aria-hidden>
                <div className="w-40 h-3 bg-gray-100 rounded-md"></div>
                <div className="flex-1 h-5 bg-gray-100 rounded-lg overflow-hidden">
                  <div className="h-full bg-gray-200/60 animate-pulse"></div>
                </div>
          ...
          <div className="space-y-3">
            {chartData.map((item, index) => (
              <div
                ...
                aria-label={`${
                  item.name
                }: ${item.value.toLocaleString()} (${item.percentage.toFixed(
                  1
                )}%)`}
              >
                <div className="flex items-center gap-2 h-9">
                  ...
                  {/* Horizontal Bar */}
                  <div className="relative flex-1 h-5 bg-gray-100 rounded-lg overflow-hidden">
                    <div
                      ...
                      aria-hidden={false}
                      aria-label={`${item.value.toLocaleString()} (${item.percentage.toFixed(
                        1
                      )}%)`}
                    >
                      {/* Subtle shimmer */}
      ...
      <div className="absolute top-4 right-4 opacity-5">
        <div
          className={`w-32 h-32 bg-gradient-to-br ${config.gradient} rounded-full blur-3xl`}
        ></div>
      </div>
    </div>
  );
